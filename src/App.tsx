    ...
    <section className="workspace">
      ...
      <div className="viewer card">
        ...
        {parsed.length ? <>
          ...
          <div className="fret-scroll"><div className={"fretboard"+(from===0?" has-open":"")} style={{'--cols':frets.length} as React.CSSProperties}>
            <div className="fret-numbers"><span></span>{frets.map(f=><span className={f===0?"open":""} key={f}>{f===0?"○":f}</span>)}</div>
            {STRINGS.map((s,si)=><div className="string-row" key={s}><b>{s}</b>{frets.map(f=>{const pitchClass=(OPEN[si]+f)%12; const toneIndex=chord?.tones.findIndex(tone=>tone.pitchClass===pitchClass)??-1; const chordTone=toneIndex>=0?chord?.tones[toneIndex]:undefined; const pos=selected.find(p=>p.string===si&&p.fret===f); const visible=!!chord&&!!chordTone&&(!optimized || !!pos); const sounding=!!currentPlayed&&currentPlayed.string===si&&currentPlayed.fret===f&&playing; return <div className={f===0?"fret open":"fret"} key={f}><span className="string"/><span className="wire"/>{visible&&<span className={'note '+colorForRole(chordTone!.role)+(sounding?' sounding':'')} title={spellTone(chord!,chordTone!)+", "+(lang==='it'?'grado':'degree')+" "+chordTone!.degree}>{spellTone(chord!,chordTone!)}</span>}</div>})}</div>)}
          </div></div>
